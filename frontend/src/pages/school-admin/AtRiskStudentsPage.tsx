...
export default function AtRiskStudentsPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      allStudents.sort((a: RiskPrediction, b: RiskPrediction) => b.overall_risk_score - a.overall_risk_score)
      setStudents(allStudents)
      
      // Calculate stats
      const calculatedStats = {
        total_students: allStudents.length,
        critical_risk: allStudents.filter((s: RiskPrediction) => s.overall_risk_level === 'CRITICAL').length,
        high_risk: allStudents.filter((s: RiskPrediction) => s.overall_risk_level === 'HIGH').length,
        medium_risk: allStudents.filter((s: RiskPrediction) => s.overall_risk_level === 'MEDIUM').length,
        low_risk: allStudents.filter((s: RiskPrediction) => s.overall_risk_level === 'LOW').length,
        avg_risk_score: allStudents.length > 0 
          ? allStudents.reduce((sum: number, s: RiskPrediction) => sum + s.overall_risk_score, 0) / allStudents.length 
          : 0
      }
          ...
          <Card className="border-blue-200 bg-blue-50">
            ...
            <CardContent>
              <div className="text-3xl font-bold text-blue-900">
                {stats?.avg_risk_score.toFixed(2) || 0}
              </div>
